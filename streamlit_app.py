...
if st.session_state.tournament_started:
    ...
    if partite:
        ...
        # Raccolta match filtrati o totali
        lista_render = []
        for idx, match in enumerate(partite_in_corso):
            tA_att, tA_port = match["teamA"]
            tB_att, tB_port = match["teamB"]
            nomi = [tA_att['name'].lower(), tA_port['name'].lower(), tB_att['name'].lower(), tB_port['name'].lower()]
            
            if not is_personale or target_user in nomi:
                lista_render.append((idx, match, "In Corso"))

        if not is_personale and partite_in_coda:
            for q_idx, match in enumerate(partite_in_coda):
                tA_att, tA_port = match["teamA"]
                tB_att, tB_port = match["teamB"]
                nomi = [tA_att['name'].lower(), tA_port['name'].lower(), tB_att['name'].lower(), tB_port['name'].lower()]
                if not is_personale or target_user in nomi:
                    lista_render.append((q_idx, match, "In Coda"))
        ...
        for idx, match, stato_match in partite_in_corso:
            tA_att, tA_port = match["teamA"]
            tB_att, tB_port = match["teamB"]
